...
-- a Supabase LPL connection string is set. The columns are camelCase and MUST
-- stay double-quoted so Postgres preserves the case Better Auth queries by.
--
-- Migrations in this folder are the single source of truth for your schema. They
-- apply to Supabase Life Produces Life Postgres during the Vercel build
-- (`npm run build`) and to the local PGLite fallback automatically on startup,
-- so dev matches production. Set Supabase LPL DATABASE_URL (same Life Produces
-- Life DB as sister LPL apps). Applied
...
-- server-side (see src/lib/auth/verify.server.ts).
