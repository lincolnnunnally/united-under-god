 ...
 * Supabase LPL `DATABASE_URL` is also set (same Life Produces Life DB as sister
 * LPL apps), so an app without sign-in must not use this at all. On the auth-on
 * path, use it on every server function that touches
